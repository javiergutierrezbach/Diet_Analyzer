...
find_diet_fit <- function(props, athlete, normal, vegan, 
...
```{r}
#find the baseline diet most similar to the inputed meal
best_fit <- find_diet_fit(Nutrients, athlete, normal,
            vegan, dessertlover, carnivore, fastfoodlover)
...
2. Sum of Squared Errors Algorithm: aiming to compare the user's meal with the baseline diets we decided to use the famus optimization method of the least squares problem. Below you can see the function of the coded version of this equation (sum_squared_errors)
...
3. Find diet fit: combining the functions from appendix 1 and 2, the find diet fit function below calculates the sum of the squared errors of the user's meal each baseline diet. The function then proceeds to print the baseline diet that had the smallest least squares solution, or in other words the baseline diet that matched to users meal.
...
analyse_meal <- function(Meal) {
  ...
  inner_join(NutrientAmounts, by = 'food_id') |> 
  inner_join(NutrientNames, by = 'nutrient_id') |> 
  filter(nutrient_name == 'PROTEIN' 
         | nutrient_name == 'FAT (TOTAL LIPIDS)' 
         | nutrient_name == 'CARBOHYDRATE, TOTAL (BY DIFFERENCE)') |> 
  mutate(new_value = nutrient_value * conversion_factor_value) |> 
  group_by(nutrient_name) |>
  summarise("value" = sum(new_value)) |>
  mutate(proportions = value * 100 / sum(value)) |> 
...
6. Quantify Nutrients: this function is used to get the amount of a particular nutrient input by the user for a particular food item, also as input. Then the result is the amount of the nutrient that the food would contain for each possible measurement in the data set. This table will not necessarily be used in the app, or maybe a variation of it will be used, but it ws made by us for developing and exploration purposes.
```{r}
# To help us quantify nutrients based on how much of the food the user ate.
quantify_nutrient <- function(food, nutrient) {
  myprops <- NutrientNames |> 
