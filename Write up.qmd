...
The quantify nutrients function is used to get the amount of a particular nutrient input by the user for a particular food item, also as input. Then the result is the amount of the nutrient that the food would contain for each possible measurement in the data set. The method again joins the nutrient tables, to query the specific nutrient the user cares about, then does the same for the food item, and by scaling the nutrient value with the conversion factor per measure, it displays how much of the nutrient the food would have for each measure. This table will not necessarily be used in the app, but it was made by us for developing and exploration purposes.
