...
Since our data set played a vital role for our project we decided to showcase it inside the diet analyzer app in the form of a DT where the user will be able to select between the twelve tables and explore them using an interactive search bar. More information about the dataset user interface will be provided in the Shiny app section of the write up.\
...
![Figure 5: Explore CanadianNutrients Dataset](Screenshots/explore%20data%20set.png)
...
Specifically, in order to get a table with the user's meal, we had to gather information about all the different food items and their nutrients, measurements, food groups and proportions. This proceedure was done by completing multiple joins. Always joining by food_id, we connected the tables food names with food groups, nutrient names, and nutrient amounts.\

Another problem that we faced when cleaning up our data was that in the french translation of the food, the column food_description_f located in the food names table, the shiny app was unable to read some of the French characters which caused issues in our search bar and was visually unpleasant. We had to tackle this problem manually since all the functions wee tried to use failed to solve this issue.
